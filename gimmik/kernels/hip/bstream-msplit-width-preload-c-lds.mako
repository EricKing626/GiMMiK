...
<%doc>
  ...
  Identical to bstream-msplit-width-preload-c, except the double-buffered B fill
  is issued as a direct global->LDS DMA (__builtin_amdgcn_load_to_lds), bypassing
  the VGPR round-trip. width (double2/double4) + preload-c. CDNA only
  (gfx90a / gfx94x). load_to_lds moves 4 bytes per call, so each element is split
  into ${ndw} dword transfers; the existing __syncthreads() barriers double as
  the DMA completion barrier. Verify on-device that the DMA has completed before
  the LDS reads.
</%doc>

<%doc>
  ...
  GRID CONVENTION: GiMMiK / PyFR size the launch grid as one thread per column
  of C, i.e. grid.x = ceil(n / blockx). A single MFMA wavefront only covers a
  16-wide column tile, so to honour that convention each 64-lane wavefront here
  sweeps TILES = warp_size/16 = 4 consecutive 16-col tiles -> blockx columns per
  block. The baked A operands do not depend on the column tile, so they are
  loaded once and reused across the sweep. (Sizing the grid as ceil(n/blockx)
  while a wavefront covered only 16 columns would compute just n/4 of C and make
  any bytes/time bandwidth estimate ~4x too high.)

  NOTE: this is a baseline MFMA kernel (no warp-specialised producer/consumer
  overlap, no LDS-staged B double-buffer). It exists to test whether the
  Matrix-Core path can approach rocBLAS on dense tet; the next step is a warp-
  specialised version, mirroring the Hopper TMA/MMA design but on CDNA3.
...
MT = 16   # MFMA M tile
KT = 4    # MFMA K tile
NT = 16   # MFMA N tile (cols of C per wavefront per MFMA)
WARP = 64 # CDNA wavefront width (this kernel is CDNA-only)
TILES = WARP // NT            # 16-col tiles swept per wavefront (= 4 on CDNA)
m_tiles = -(-m_ // MT)
k_tiles = -(-k_ // KT)
...
% endif
    ...
    const int wbase  = wave*${NT*TILES};           // first C/B column this wavefront owns (covers NT*TILES)
    if (wbase >= n) return;
    ...
    // A is independent of the column tile, so it is loaded once and reused.
% for mt in range(m_tiles):
%  for kt in range(k_tiles):
...
% endfor
    ...
    // Sweep TILES=${TILES} consecutive 16-col C tiles -> ${NT*TILES} cols/block.
% for ct in range(TILES):
    {
        const int n_col = wbase + ${ct*NT} + bcol;

<%inherit file='base'/>
<%doc>
  bstream-msplit-width-lds
  = repo 的 bstream-msplit-width(naked, 無 preload-c)+ B 雙緩衝填充改成
    global -> LDS 直送(__builtin_amdgcn_load_to_lds),繞過 VGPR round-trip。
  與 bstream-msplit-width-preload-c-lds 的差別:本檔「不」preload C(沿用 naked
    的 vmul-first / vmadd 累加),用來隔離出 LDS B-fill 的單獨效果。
  gfx942 的 load_to_lds 每次僅 1/2/4 byte,故每元素拆成 ${ndw} 次 size=4 傳輸
    (double=2、double2=4);用 C 迴圈以免 mako loop.index 被內層覆蓋。
...
% else:
#error "bstream_msplit_width_lds only supports width=2 or width=4"
% endif
  ...
  % for cid, mcx in enumerate(mx):
    ...
    % for kx in bchunks[bb]:
      ...
      % for j, jx in enumerate(A[mcx, kx]):
        ...
        nt_store_c(&c[i + ${mcx[j]}*ldc], gimmik_vadd(c[i + ${mcx[j]}*ldc], csub[${j}]));
        % elif kx == alix[mcx[j]]:
        nt_store_c(&c[i + ${mcx[j]}*ldc], gimmik_vadd(csub[${j}], gimmik_vmul(${beta}, c[i + ${mcx[j]}*ldc])));
        % endif
      % endfor
    % endfor
